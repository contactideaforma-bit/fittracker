...
export default function TabataPage() {
  ...
  function nextPhase() {
    ...
    if (phase === 'effort') {
      // Après effort → pause (sauf dernière série du dernier tour)
      ...
      if (serie < series) {
        ...
        // Fin des séries → repos entre tours
        if (tour < tours) {
  ...
  const phaseConfig = {
    effort: { label: 'EFFORT',  bg: 'bg-red-500',    text: 'text-red-600',    ring: 'border-red-400' },
    pause:  { label: 'PAUSE',   bg: 'bg-green-500',  text: 'text-green-600',  ring: 'border-green-400' },
    repos:  { label: 'REPOS',   bg: 'bg-blue-500',   text: 'text-blue-600',   ring: 'border-blue-400' },
    idle:   { label: 'PRÊT',    bg: 'bg-zinc-400',   text: 'text-zinc-600',   ring: 'border-zinc-300' },
  }

  const pc = phaseConfig[phase]

  const input = "w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white text-center font-mono"
        ...
        <div className="card flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="1" max="20" className={input} value={series}
                onChange={e => setSeries(parseInt(e.target.value) || 1)} />
            </div>
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">Nombre de tours</label>
              <input type="number" min="1" max="10" className={input} value={tours}
                onChange={e => setTours(parseInt(e.target.value) || 1)} />
            </div>
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">Durée effort (sec)</label>
              <input type="number" min="5" max="300" className={input} value={effortSec}
                onChange={e => setEffortSec(parseInt(e.target.value) || 20)} />
            </div>
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">Durée pause (sec)</label>
              <input type="number" min="5" max="120" className={input} value={pauseSec}
            ...
            <div className="col-span-2">
              ...
              <input type="number" min="10" max="300" className={input} value={reposSec}
                onChange={e => setReposSec(parseInt(e.target.value) || 60)} />
            </div>
          </div>
          ...
          <div className="bg-zinc-50 rounded-lg p-3 text-xs text-zinc-500 flex flex-col gap-1">
            ...
            <div className="flex justify-between"><span>Durée totale estimée</span><span className="font-mono font-medium">{totalMin} min {totalSecs % 60} sec</span></div>
          </div>
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="w-full flex flex-col gap-2">
            ...
            <div className="w-full h-2 bg-zinc-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-1000 ${pc.bg}`}

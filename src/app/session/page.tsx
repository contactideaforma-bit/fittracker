  ...
  return (
    <div className="card border-2 border-violet-200 flex flex-col gap-3">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => setMode('duration')} className={`flex-1 py-1.5 rounded-lg text-xs font-medium border transition-colors ${mode === 'duration' ? 'bg-violet-600 text-white border-violet-600' : 'border-zinc-200 text-zinc-500'}`}>Durée</button>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="card flex flex-col items-center gap-4 py-8">
          ...
          <button onClick={togglePause}
            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full transition-colors ${paused ? 'bg-violet-100 text-violet-700 hover:bg-violet-200' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea id="notes" value={notes} onChange={e => setNotes(e.target.value)}
            ...
            rows={4} className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-violet-500 resize-none" />
        </div>
